# -*- coding: utf-8 -*-
"""Yurih Sales - Atividade_avaliativa_SQL.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1bxuimHdr8TtvrDHDicyKbU_2bTnTqp3R
...
query = """
SELECT
  C.CPF as CPF, C.NomeCon as Nome, C.UFNascCon as Estado
FROM
  Consultor as C
WHERE
  C.UFNascCon != 'SP'
...
SELECT DISTINCT
  L.UFLoc as UFs
FROM
  Local as L, Viagem as V
WHERE
  V.Sigla = 'RecSys' AND V.Destino = L.NomeLoc
"""
...
query = """
SELECT DISTINCT
  Projeto.Sigla as Sigla, Projeto.NomePro as Nome
FROM
  Projeto LEFT JOIN Equipe
  ON Projeto.Sigla = Equipe.Sigla
WHERE
  Equipe.Sigla IS NULL
"""
...
SELECT DISTINCT
  Projeto.Sigla as Sigla, Projeto.NomePro as Nome
FROM
  Projeto, Equipe, Consultor
WHERE
  Projeto.Sigla = Equipe.Sigla AND
  Equipe.CPF = Consultor.CPF AND
  Consultor.CPF NOT IN
    (
      SELECT
        Consultor.CPF
      From
        Consultor
      Where
        strftime('%Y',Consultor.DataNascCon) < '1987'
    )
"""
...
query = """
SELECT DISTINCT
  Consultor.CPF as CPF, Consultor.NomeCon as Nome
FROM
  Consultor
WHERE
  Consultor.CPF IN (
    SELECT
      Consultor.CPF
    FROM
      Consultor, Viagem, Local
    WHERE
      Consultor.CPF = Viagem.CPF AND
      Viagem.Origem = Local.NomeLoc AND
      Consultor.UFNascCon = Local.UFLoc
  ) OR
  Consultor.CPF IN (
    SELECT
      Consultor.CPF
    ...
    WHERE
      Consultor.CPF = Viagem.CPF AND
      Viagem.Destino = Local.NomeLoc AND
      Consultor.UFNascCon = Local.UFLoc
  )
"""
...
query = """
SELECT DISTINCT
  Local.NomeLoc as Local, Local.UFLoc as Estado
FROM
  Local
Where
  Local.NomeLoc IN (
    SELECT DISTINCT
      Local.NomeLoc
    FROM
      Local LEFT JOIN Viagem
      ON Local.NomeLoc = Viagem.Origem
...
SELECT DISTINCT
  Projeto.Sigla as Sigla, Projeto.NomePro as Nome
FROM
  Projeto, Viagem, Local
WHERE
  Projeto.Sigla = Viagem.Sigla AND
  Viagem.Destino = Local.NomeLoc AND
  Local.UFLoc = 'CA' AND
  strftime('%Y',Viagem.DataViagem) >= '2019'
"""
...
WHERE
  Consultor.CPF = Viagem.CPF AND
  (Viagem.CPF,Viagem.Sigla) NOT IN
  (
    SELECT
      Equipe.CPF, Equipe.Sigla
    FROM
      Equipe
  )
"""
...
WHERE
  ...
  (
    ...
    FROM
      Projeto, Equipe
    WHERE
      Equipe.Sigla = Projeto.Sigla AND
      strftime('%Y',Projeto.DataInicio) = '2019'
  )
"""
